...
class FilterEngine:
    ...
    def _sync_gmgn_lp_burn(self, t: Token) -> Token:
        if t.gmgn_lp_burned:
            # Sudah dikonfirmasi GMGN, pastikan lp_burn 100%
            if t.lp_burn < 100:
                t.lp_burn = 100.0
                log.info(f"Sync GMGN LP burn: 100% for {t.mint[:12]}")
        elif t.lp_burn >= 95:
    ...
    def _analyze_gmgn_holders(self, t: Token, data: dict) -> Token:
        ...
        if t.top10_source == "GMGN" and t.top10_pct > 0:
            ...
            # Coba ambil top10 dari GMGN data (root level)
                ...
                if raw is not None:
                    try:
                        ...
                        if 0 < v <= 100:
                            ...
                            t.top10_source = "GMGN"
                            log.info(f"Filter GMGN top10 (root): {t.top10_pct}% for {t.mint[:12]}")
                    ...
                    if raw is not None:
                        try:
                            ...
                            if 0 < v <= 100:
                                t.top10_pct    = round(v, 1)
                                t.top10_source = "GMGN"
                                log.info(f"Filter GMGN top10 (dev): {t.top10_pct}% for {t.mint[:12]}")
                        except (ValueError, TypeError):
                            pass
                        break
