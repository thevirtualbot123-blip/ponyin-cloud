"""
filter_engine.py — PONYIN AI AGENT v3.1 (FIXED)
=================================================
Fix:
  - Token dataclass: semua field ada dengan default value
  - sizing_note selalu diinisialisasi
...
class Token:
    ...
    sizing_note: str = ""          # FIX: selalu ada dengan default ""
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "plan": self.plan,
            "sizing_note": self.sizing_note,  # FIX: selalu ada
...
class FilterEngine:
    ...
    def run(self, t: Token) -> Token:
        """Main filter pipeline. Selalu return Token yang valid."""
        try:
            t = self._classify(t)
            ...
            import logging
            logging.getLogger("PONYIN.Filter").error(f"Filter error for {t.mint[:12]}: {e}")
            t.verdict = "ERROR"
            t.flags = 99
            t.sizing_note = "Error saat filter — skip manual"
        return t

    # ── Classify token type ───────────────────────────────
    def _classify(self, t: Token) -> Token:
        if t.mc < 100_000:
            t.position_type = "LOWCAP"
        elif t.mc < 2_000_000:
            t.position_type = "MIDCAP"
        else:
            t.position_type = "HIGHCAP"
        return t
    ...
    def _detect_wash_trading(self, t: Token) -> Token:
        ...
        if t.vol1h > 5000 and total_txn > 0:
            avg_tx = t.vol1h / total_txn
            if avg_tx > 2000:
                reasons.append(
                    f"Vol/MC ratio {t.vol1h/t.mc:.1f}x (${t.vol1h:,.0f} vol / ${t.mc:,.0f} MC) — anomali tinggi"
                    if t.mc > 0 else f"avg tx ${avg_tx:,.0f} — tidak natural"
                )

        if t.vol1h > 1000 and total_txn == 0:
            reasons.append(f"vol ${t.vol1h:,.0f} tapi 0 txn")

        if t.mc > 0 and t.vol1h > 0 and t.vol1h / t.mc > 3.0:
            reasons.append(
                f"Vol/MC ratio {t.vol1h/t.mc:.1f}x (${t.vol1h:,.0f} vol / ${t.mc:,.0f} MC) — anomali tinggi"
            )

        if t.chg1h > 50 and total_txn < 15 and t.mc > 30000:
            reasons.append(f"pump {t.chg1h:+.0f}% dengan {total_txn} txn — tidak organik")

        if t.liq > 0 and t.vol1h > t.liq * 5:
            reasons.append(f"vol ${t.vol1h:,.0f} = {t.vol1h/t.liq:.0f}x likuiditas — impossible")

        t.wash_trading_flag   = len(reasons) > 0
        t.wash_trading_reason = " | ".join(reasons) if reasons else ""
        return t

    # ── Cluster / Hidden whale ────────────────────────────
    def _detect_cluster(self, t: Token) -> Token:
        ...
        if len(pcts) >= 5:
            avg = sum(pcts[:10]) / min(10, len(pcts))
            devs = [abs(p - avg) for p in pcts[:10] if p > 0]
            if devs and sum(devs)/len(devs) < 0.3 and avg > 1.0:
                reasons.append("distribusi terlalu uniform — kemungkinan 1 entitas")
                risk = "HIGH"

        top10_total = sum(pcts[:10])
        if top10_total > 70:
            reasons.append(f"top10 {top10_total:.1f}% — extreme")
            risk = "CRITICAL"
        elif top10_total > 55:
    ...
    def _detect_dev_farm(self, t: Token) -> Token:
        ...
        if t.lp_burn == 0:
            reasons.append("LP tidak di-burn")
            risk = "HIGH"
        elif t.lp_burn < 50:
        ...
        dev_kws = ["dev", "creator", "deployer", "farm", "bundle", "sniper", "insider"]
        ...
        if t.age_hours < 0.5 and t.chg1h > 200 and (t.buys1h + t.sells1h) < 50:
            reasons.append(f"pump {t.chg1h:.0f}% dalam <30 menit")
            if risk == "LOW": risk = "MEDIUM"
    ...
    def _detect_smart_money(self, t: Token) -> Token:
        ...
        t.smart_money_pct     = round(total_pct, 1)
        return t
    ...
    def _timing_score(self, t: Token) -> Token:
        ...
        if 20 <= hour or hour < 2:    base, reason = 90, "US prime time (20:00-02:00 UTC)"
        elif 13 <= hour < 20:          base, reason = 75, "EU/US overlap (13:00-20:00 UTC)"
        elif 10 <= hour < 13:          base, reason = 65, "EU peak (10:00-13:00 UTC)"
        elif 6  <= hour < 10:          base, reason = 55, "EU morning (06:00-10:00 UTC)"
        elif 2  <= hour < 6:           base, reason = 20, "Dead hours (02:00-06:00 UTC)"
        else:                           base, reason = 40, f"Hour {hour} UTC"

        t.timing_score  = max(0, min(100, base + weekend))
        t.timing_reason = reason + (" (weekend)" if dow >= 5 else "")
        return t
    ...
    def _bounce_potential(self, t: Token) -> Token:
        ...
        if t.chg24h < -30 and t.vol24h > 5000:
            signals.append(f"correction {t.chg24h:.0f}% tapi vol ${t.vol24h:,.0f}")
            score += 2
        if t.liq > 5000:
            signals.append(f"liq ${t.liq:,.0f} masih ada"); score += 1
        ...
        t.bounce_reason    = " | ".join(signals) if signals else ""
        return t
    ...
    def _apply_filters(self, t: Token) -> Token:
        ...
        if t.cluster_risk == "CRITICAL":
            bad("Cluster CRITICAL", t.cluster_reason[:80], "CRITICAL")

        # Liq trap
        if t.liq_trap_risk:
            ratio = t.liq_mc_ratio
            bad("Liq Trap", f"Liq/MC {ratio:.1%} — suspicious", f"{ratio:.1%}")

        # S1 Authority
        if t.mint_auth:
            bad("S1 Mint Auth", "AKTIF — dev bisa cetak token → dilusi", "ACTIVE")
        elif t.freeze_auth:
            bad("S1 Freeze Auth", "AKTIF — dev bisa freeze holder", "ACTIVE")
        else:
            danger = [n for (lvl,n,d,v) in t.rc_risks
                      if lvl=="danger" and any(k in n.lower()
                      for k in ["bundle","honeypot","sniper"])]
            if danger:
                bad("S1 Bundle/Honeypot", danger[0], "DANGER")
            else:
                lp = f"{t.lp_burn:.0f}% burned" if t.lp_burn > 0 else "N/A"
                ok("S1 Authority OK", f"Revoked | LP {lp}", "Aman")

        # S2 MC & Liq
        if t.mc <= 0:
            bad("S2 MC", "Data tidak ada", "N/A")
            ...
            ratio = t.liq_mc_ratio
            if ratio < 0.04:
                bad("S2 Liq/MC", f"{ratio:.1%} < 4%", f"{ratio:.1%}")
            else:
                ok("S2 MC & Liq", f"MC ${t.mc:,.0f} | Liq ${t.liq:,.0f}", f"Ratio {ratio:.1%}")

        # S3 Top holders
        if t.top10_pct == 0:
            info("S3 Top10", f"N/A ({t.top10_source})", "Cek Solscan manual")
        elif t.top10_pct > 70:
            bad("S3 Top10", f"{t.top10_pct:.1f}% > 70% → CABAL/BUNDLE → dump risk", f"{t.top10_pct:.1f}%")
        elif t.top10_pct > cfg.MAX_TOP10_PCT:
            bad("S3 Top10", f"{t.top10_pct:.1f}% > {cfg.MAX_TOP10_PCT}%", f"{t.top10_pct:.1f}%")
        else:
            ok("S3 Top10", f"{t.top10_pct:.1f}% ({t.top10_source})", "Distribusi sehat")

        # S4 Risk & LP
        rn = t.risk_norm
        if rn > 7:
            bad("S4 Risk", f"Score {rn}/10 [{t.risk_label.upper()}] — BERBAHAYA", f"{rn}/10")
        elif rn > cfg.MAX_RISK_NORM:
            bad("S4 Risk", f"Score {rn}/10 > max {cfg.MAX_RISK_NORM}", f"{rn}/10")
        elif 0 < t.lp_burn < 80:
        ...
        if t.smart_money_present:
            ok("Smart Money", f"{t.smart_money_pct:.1f}% non-insider", "GAKE masih hold ✓")
        ...
        if t.age_hours > 0:
            ...
            elif t.bounce_potential: info("Age", age, f"Old tapi bounce: {t.bounce_reason[:40]}")
            else:                    info("Age", age, "Old — cek thesis")

        hh = t.holder_health
        if hh >= 70:   ok("Holder Health", f"{hh}/100", "Sehat")
        elif hh >= 50: info("Holder Health", f"{hh}/100", "Moderate")
        else:          info("Holder Health", f"{hh}/100", "Concern")
    ...
    # ── Build trading plan + sizing ───────────────────────
    def _build_plan(self, t: Token) -> Token:
        """Build plan. sizing_note SELALU diisi (default string)."""
        cfg = self.cfg
        ...
        # Param berdasarkan position type
        if t.position_type == "LOWCAP":
            ...
            note = f"MIDCAP: max {max_p*100:.0f}% portfolio (~{cfg.PORTFOLIO_SOL*max_p:.3f} SOL). Bisa hold lebih lama."
        else:
        ...
        t.sizing_note = note  # Selalu di-set
        return t
