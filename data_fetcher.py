"""
data_fetcher.py — PONYIN AI AGENT v6.0 (GMGN API integration)
"""
...
class DataFetcher:
    ...
    def _apply_gmgn_data(self, t: Token, data: dict) -> Token:
        """Ambil semua data penting dari GMGN."""
        t.gmgn_data = data
        ...
        # Fresh wallets
        fw = data.get("fresh_wallet_rate")
        if fw is not None:
            t.fresh_wallet_rate = float(fw)

        # Rat trader (insider)
        rat = data.get("rat_trader_amount_rate")
        if rat is not None:
            t.rat_trader_rate = float(rat)
    ...
    def _apply_rugcheck(self, t: Token, rc: dict) -> Token:
        ...
        if not hasattr(t, 'gmgn_data') or not t.gmgn_data:
